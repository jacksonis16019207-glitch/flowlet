...
- DB 変更を含む場合は、`docs/ops/db-migration-rules.md` を確認し、少なくとも次を実装前に確定する: `既存 migration を編集しないこと`、`新しい version を追加すること`、`seed を migration に混ぜないこと`、`データ保持が必要なら backup schema と移行手順を用意すること`。
...
- DB migration を追加する場合は、`1 migration 1 関心事`、`migration 名で内容を追えること`、`テーブル再作成か局所変更かの判断理由をどこへ残すか` まで計画に含める。
...
- DB 変更後は、必要に応じて `docs/ops/setup.md`、関連仕様メモ、`docs/ops/db-migration-rules.md`、対象 Skill の更新要否まで確認する。
...
- DB migration を含むタスクでは [docs/ops/db-migration-rules.md](/C:/Users/jacks/Documents/flowlet/docs/ops/db-migration-rules.md) を必ず参照する。
